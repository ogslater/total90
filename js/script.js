...
document.addEventListener('DOMContentLoaded', () => {
  /* fetch('ids.json')
    ...
    fetch(`http://localhost:3000/api/felinas`).then(res => res.json().then(json => {
...
function atualizarprofile(index, userData) {
  ...
  if(userData.user.id == '1025968255497674813' && !userData.connected_accounts.find(x => x?.type == 'instagram')) userData.connected_accounts.push({ type: 'instagram', name: 'mxanaju' })
  if(userData.user.id == '421389279114166282' && !userData.connected_accounts.find(x => x?.type == 'instagram')) userData.connected_accounts.push({ type: 'instagram', name: 'wtfcbebea' })
  if(userData.user.id == '707763772726575206' && !userData.connected_accounts.find(x => x?.type == 'instagram')) userData.connected_accounts.push({ type: 'instagram', name: 'vdchique' })
  if(userData.user.id == '1025968255497674813' && !userData.connected_accounts.find(x => x?.type == 'instagram')) userData.connected_accounts.push({ type: 'instagram', name: 'mxanaju' })
  if(userData.user.id == '570975728334798860' && !userData.connected_accounts.find(x => x?.type == 'instagram')) userData.connected_accounts.push({ type: 'instagram', name: 'wbcrwy' })
  if(userData.user.id == '1182544489445863468' && !userData.connected_accounts.find(x => x?.type == 'instagram')) userData.connected_accounts.push({ type: 'instagram', name: 'beelwk' })
  if(userData.user.id == '180890254247460867' && !userData.connected_accounts.find(x => x?.type == 'instagram')) userData.connected_accounts.push({ type: 'instagram', name: 'bnz7w' })
  if(userData.user.id == '1196271734450561175' && !userData.connected_accounts.find(x => x?.type == 'instagram')) userData.connected_accounts.push({ type: 'instagram', name: 'nuncarendo' })
